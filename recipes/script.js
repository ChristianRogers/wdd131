...
function displayRecipes(recipesToDisplay) {
  for (let i = 0; i < recipesToDisplay.tags.length; i++) {
    recipesToDisplay.tags[i] = `<button>${recipesToDisplay.tags[i]}</button>`;
  }
  tags = recipesToDisplay.tags.join(" ");

  recipe.innerHTML += `<section class="recipe"><img src="${
    recipesToDisplay.image
  }" alt="${recipesToDisplay.name}" />
  
  <div>
    ${tags}
    <h2>${recipesToDisplay.name}</h2>
    <span class="rating" role="img" aria-label="Rating: ${
      recipesToDisplay.rating
    } out of 5 stars">
      ${"⭐".repeat(recipesToDisplay.rating)}
      ${"☆".repeat(5 - recipesToDisplay.rating)}
    </span>

    <p class="hidden">
      ${recipesToDisplay.description}
    </p>
  </div>
  </section>`;
}

displayRecipes(recipes[randomNum]);
...
function searchRecipes() {
  ...
  if (sortedRecipes.length !== 0) {
    ...
    for (let i = 0; i < sortedRecipes.length; i++) {
      displayRecipes(sortedRecipes[i]);
    }
  } else {
